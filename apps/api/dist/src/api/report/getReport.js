...
const validateInputPopulateLocals = async function (req, res, next) {
...
const calculateFreeExtensions = async (_, res, next) => {
...
const calculatePaidExtensions = async (_, res, next) => {
    const { dateFrom, dateTo, report } = res.locals;
...
const calculateNewSubs = async (_, res, next) => {
    const { dateFrom, dateTo, report } = res.locals;
...
const calculateRepayments = async (_, res, next) => {
    const { dateFrom, dateTo, report } = res.locals;
...
const calculateTotalExpenses = async (_, res, next) => {
    const { dateFrom, dateTo, report } = res.locals;
    if (!report.freeExtensions ||
        !report.paidExtensions ||
        !report.newSubs ||
        !report.repayments)
...
const calculateTotalIncome = async (_, res, next) => {
    const { dateFrom, dateTo, report } = res.locals;
