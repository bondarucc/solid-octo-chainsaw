...
export const promoteSub = async ({ params: { externalId }, body }, res) => {
    ...
    if (sub.user)
        throw "this sub is a partner already";
    const updatedSub = await prisma.$transaction(async (trn) => {
        if (body.type === "existingUser") {
            ...
            if (existingUser.subExternalId)
                throw "selected user is already assigned to another sub";
            if (sub.referalDetails?.attractorUserId === existingUser.login)
                throw "self referring";
        }
        else {
